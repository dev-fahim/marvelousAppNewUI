...
import { ExpenditureRecordGETModel, LoanExpendGETModel } from './../../../service/models';
...
export class DebitEditComponent implements OnInit {
  expenditure_data: LoanExpendGETModel = {
    "id": 0,
        "expend_heading_name": "",
        "added_by": "",
        "loan": {
  ...
  ngOnInit() {
    ...
    this._loanService.get_specific_record(this.uuid)
      .subscribe(
        (result) => {
          this.loading = false;
  ...
  onSubmit() {
    this.loading = true;
    return this._loanService.update_record(this.form.value, this.uuid)
      .subscribe(
        (result) => {
          this.loading = false;
  ...
  onDelete() {
    ...
    this._loanService.delete_record(this.uuid, this.form.value)
      .subscribe(
        (result) => {
